function ValidateSudoku2(sudoku){   // START OF FUNCTION <===
    ...
    // Return
      ...
      if(finalArray === attempt){return true}

      let attempt2 = attempt.reduce((acc,val) => {
        return acc.concat(val)
      }, [])
      console.log(attempt2.join(''))
      console.log(final.join(''))

      if(final.join('') === attempt2.join('')){return true}
      else {return false}

      // attempt2.forEach((e,i) => {
      //   if(final[i] !== attempt2[i]){return false}
      // })
    ...
    export default ValidateSudoku2;